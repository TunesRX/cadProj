 ...
 */
__global__ void areaFilter(int *out, int *img, int imgw, int imgh, int *filter,
                           int blockIdxY) {
  ...
  int line = blockIdxY * blockDim.x + threadIdx.y;

  if (line < imgh && col < imgw) {
    // printf(" col %d line %d\n",col,line);
    /*int blockX = 0;
     int blockY = 1;
     int init = blockY*blockDim.x -1 ;
     if (threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.x==blockX
     &&blockIdxY==blockY){ for(int i = init; i < init +4; i++){ for( int j = 0;
     j < imgw; j ++){ int idx = 3 * (i * imgw + j); printf("%d %d %d  ",
    ...
    // copia lado inferior
    int nbH = (imgh + blockDim.x - 1) / blockDim.x;
    if (threadIdx.y == blockDim.x - 1 && blockIdxY != nbH - 1) {
      int idx = 3 * ((line + 1) * imgw + col);
      tempCol = threadIdx.x + 1;
      tempLine = sharedSz - 1;
      updateArray(img, temp, idx, tempLine, tempCol);
    ...
    if (threadIdx.x == 0 && threadIdx.y == 0) {
      ...
      if (blockIdxY != nbH - 1 && col != 0) {
        idx = 3 * ((blockIdxY * blockDim.x + blockDim.x) * imgw + (col - 1));
        updateArray(img, temp, idx, sharedSz - 1, 0);
      }

      if (blockIdxY != nbH - 1 && blockIdx.x != nbW - 1) {
        idx = 3 * ((blockIdxY * blockDim.x + blockDim.x) * imgw +
    ...
    int blockY = 1;
    if (threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.x==blockX
    &&blockIdxY==blockY){ for(int i = 0; i < sharedSz; i++){ for( int j = 0; j <
    ...
    for (int l = threadIdx.y; l < threadIdx.y + 3; l++)
      for (int c = threadIdx.x; c < threadIdx.x + 3; c++) {
        ...
        int line1 = blockIdxY * blockDim.x + l;
    ...
    line = blockIdxY * blockDim.x + threadIdx.y;
    idx = 3 * (line * imgw + col);
    out[idx] = r / n;
    out[idx + 1] = g / n;
    out[idx + 2] = b / n;
 ...
 */
__global__ void pointFilter(int *out, int *img, int imgw, int imgh, float alpha,
                            int blockIdxY) {
  int col = blockIdx.x * blockDim.x + threadIdx.x;
  int line = blockIdxY * blockDim.x + threadIdx.y;
...
int main(int argc, char *argv[]) {
  ...
  cudaStream_t streams[by];
  int offset;
  int offsetFinal;

  for (int blockY = 0; blockY < by; blockY++) {
    cudaStreamCreate(&streams[blockY]);
    offset = 3 * ((blockY * blockSize.y - 1) * imgw);
    if (offset < 0)
      offset = 0;

    offsetFinal = 3 * ((blockY * blockSize.y - 1 + 34) * imgw);
    if (offsetFinal > 3 * imgh * imgw)
      offsetFinal = 3 * imgh * imgw;

    cudaMemcpyAsync(&d_a[offset], &img[offset],
                    (offsetFinal - offset) * sizeof(int),
                    cudaMemcpyHostToDevice, streams[blockY]);

    areaFilter<<<bx, blockSize, 0, streams[blockY]>>>(d_b, d_a, imgw, imgh,
                                                      d_filter, blockY);

    pointFilter<<<bx, blockSize, 0, streams[blockY]>>>(d_b, d_b, imgw, imgh,
                                                       alpha, blockY);

    cudaMemcpyAsync(&out[offset], &d_b[offset],
                    (offsetFinal - offset) * sizeof(int),
                    cudaMemcpyDeviceToHost, streams[blockY]);
  }
